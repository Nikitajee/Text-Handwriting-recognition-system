...
class File():
    ...
    def saveFile(self):
        try:
            ...
            f = open(self.filename, 'w',parent=self.root)
            f.write(t)
            f.close()
        except:
            self.saveAs()
    ...
    def saveAs(self):
        f = asksaveasfile(mode='w', defaultextension='.txt',parent=self.root)
        t = self.text.get(0.0, END)
        try:
            f.write(t.rstrip())
        except:
                showerror(title="Oops!", message="Unable to save file...",parent=self.root)

    def openFile(self):
        f = askopenfile(mode='r',parent=self.root)
    ...
    def quit(self):
        entry = askyesno(title="Quit", message="Are you sure you want to quit?",parent=self.root)
        if entry == True:
            self.root.destroy()
